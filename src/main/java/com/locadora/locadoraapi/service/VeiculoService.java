...
public class VeiculoService {
    ...
    public Collection<? extends Veiculo> getVeiculoByTipoCarro(int tipoCarro) {
         return veiculoRepository.findByTipo(TipoCarroEnum.values()[tipoCarro-1] ); // 1 - passeio, 2 - SUV, 3 - pickup, subtrai-se um pois o indice inicia em zero
    }
    ...
    public void depreciarVeiculos(int tipo, double taxaDepreciacao) {
        ...
        if(tipo == 0){
            this.veiculoRepository.findAll().forEach( (veiculo) -> {
             ...
             this.veiculoRepository.findAllVeiculosByType(TipoVeiculoClassHelper.getTipoVeiculoClass(tipo)).forEach( (veiculo) -> {
                veiculo.depreciar(taxaDepreciacao);
                this.veiculoRepository.save(veiculo);
        });
        }
    ...
    //Este é o problema 1
    public Aluguel registrarAluguel(String placa, Date dataInicio, int dias, Long cpf)  throws VeiculoNaoCadastrado, VeiculoAlugado, ClienteNaoCadastrado {
        return new Aluguel();
    }
    
    //Este é o problema 2
    public List<Veiculo> aumentarDiaria(int tipo, double taxaAumento)  throws TaxaAumentoForaIntervalo, TipoVeiculoDesconhecido{
        return null;
    }
